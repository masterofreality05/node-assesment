...
/** Register user with data. Returns new user data. */
  ...
  static async authenticate(username, password) {
    console.log("inside autenticate User method password is: " + password + " needs to bcrypt compared to one stored in db")
    const result = await db.query(
      `SELECT username,
                password,
                first_name,
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      console.log("password compare successful")
      return user;
  ...
  static async getAll() {
    ...
    const result = await db.query(
      //selects all columns except for isadmin.
  ...
  static async registerAdmin(username, password, first_name, last_name, email, phone, admin) {
    const duplicateCheck = await db.query(
      `SELECT username 
        FROM users 
        WHERE username = $1`,
